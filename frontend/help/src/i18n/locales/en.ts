...
export default {
  ...
  nav: {
    ...
    acBiAssignment: 'Audience Assignment',
    acBiDataViewAssignment: 'Data View Assignment',
  ...
  upTasksToClaimGuide: {
    ...
    flowTitle: 'Order of work',
    flow1: 'Open To Do',
    flow2: 'Find a role request with an empty Current Assignee cell, or click Claim all',
    ...
    unclaimSample: 'releases the hold; Current Assignee becomes empty and another member can Claim it',
    claimedBySample: 'Current Assignee column on To Do; empty only on a free role request',
  ...
  formUploadGuide: {
    ...
    runtimeBody:
      'When Multi is off, drop or pick one file; the saved value is a single URL. When Multi is on, drop several files onto the dashed box, or click it and select several files in one go (Ctrl or Shift click in the file picker). Uploaded files appear as small cards inside the box, sorted by name. Click a card to open a File details drawer. Next to the file name are Preview and, unless Can not download is on, Download. File Description has a Save button — it is not saved on blur. Callback URL and Auto Send to FileNet are hidden for now. A sub-table add/edit dialog uses the same cards and drawer. A sub-table list cell still shows the first file name and +N for the rest, for example report.pdf +2. If a companion filename column is configured on the Upload field, the original names are written there, joined with a semicolon and space. Send Email attachments from a FILE field include every stored file. Basic Upload on the same form uses this same drawer.',
