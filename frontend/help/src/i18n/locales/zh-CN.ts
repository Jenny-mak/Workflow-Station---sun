...
export default {
  ...
  upTasksToClaimGuide: {
    ...
    leaderBody:
      '成员和 Leader 是「某个业务单元 + 某个角色」上的身份，不是全平台标记。成员只能认领和取消认领自己持有的请求。同一角色的 Leader 可以强制释放别人的持有，也可以把请求重新分配给同一角色的其他成员（空闲或已被他人持有均可）。该业务单元的审批人和系统管理员（SYS_ADMIN）也有同样的强制释放和重新分配权限。审计员没有。强制释放和重新分配是两件独立操作：强制释放把请求退回认领池；重新分配指定给所选成员，不会退回池。强制释放前会确认：认领人变为空，其他成员可以再认领。重新分配会打开对话框，从同一角色成员中选择；变更历史和管理中心 → 用户门户审计日志会记录谁在何时从谁改到谁。管理中心 → 用户管理会在每个业务单元角色上显示成员或 Leader。组织架构 → 准入角色列出每个角色的 Leader，方便找到谁能拆卡住的锁或改派。',
    ...
    failTitle: '认领失败时',
    failBody:
      '若其他人已经持有、你已不在任务创建时写入的角色里、或引擎不可用，认领会失败。刷新列表：「认领人」列显示当前持有人。若你不是持有人，取消认领会失败。一键取消认领会跳过同事持有并汇总跳过或失败条数。若你不是该角色的 Leader、该业务单元的审批人、或系统管理员，强制释放和重新分配也会失败。重新分配还会在所选用户不在同一角色池、或已是当前持有人时失败。一键认领遇到失败行会继续后面的批次，最后汇总失败条数。',
  },
  taskDelegateGuide: {
    pageTitle: '委托任务',
    crumb: '用户门户 · 任务 · 待办',
